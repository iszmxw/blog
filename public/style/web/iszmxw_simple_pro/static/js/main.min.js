var isOn = true;
$(".p-header-box-menu-btn").on("click", "#onOffMenu", function () {
    if (isOn) {
        isOn = false;
        offMenu()
    } else {
...
function onMenu() {
    ...
    $(".p-left-logo-box-text img").show("fast");
    if ($(".p-left-logo-box-text img").length > 0) {
        $(".p-left-logo-box-text i").hide("fast")
    }
    $(".p-left-box .layui-nav-tree .layui-nav-more").show("fast");
    $(".p-left-box .layui-nav .layui-nav-item a").animate({
        padding: "0 20px"
    }, "fast");
    $(".p-left-user-img-box").animate({
        width: "100px",
        height: "100px"
    }, "fast");
    $(".p-left-logo-box-text").animate({
        paddingLeft: "10px"
    }, "fast");
    $(".p-left-box").animate({
        width: "220px"
    }, "fast");
    $(".p-left-box-2").animate({
        width: "237px"
    }, "fast");
    $(".p-right-box").animate({
        paddingLeft: "220px"
...
function offMenu() {
    ...
    $(".p-left-logo-box-text img").hide();
    $(".p-left-user-hitokoto-box").hide();
    $(".p-left-menu-nav-txt").hide();
    $(".p-left-bottom-box").hide();
    $(".p-left-logo-box-text i").show("fast");
    $(".p-left-box .layui-nav-tree .layui-nav-more").hide();
    $(".p-left-box .layui-nav .layui-nav-item a").css({
        padding: "0 13px"
    });
    $(".p-left-user-img-box").css({
        width: "30px",
        height: "30px"
    });
    $(".p-left-logo-box-text").animate({
    ...
    $(".p-left-box .layui-nav .layui-nav-child dd").css({
        "display": "none"
    })
}

$(".p-left-logo-box").on("click", ".p-left-logo-box-text", function () {
    isOn = true;
    onMenu()
});
$(".layui-nav-item").on("click", ".is-two-menu", function () {
    isOn = true;
    onMenu()
});
$(".p-left-menu-box").on("mouseenter", ".layui-nav-item", function () {
    if (!isOn) {
        var tip = $(this).children(".p-nav-menu-btn").children(".p-left-menu-nav-txt").text();
        var that = this;
        layer.closeAll("tips");
        layer.open({
            type: 4,
...
$(".p-left-menu-box").on("mouseleave", ".layui-nav-item", function () {
    layer.closeAll("tips")
});
$("#skPlayer").on("mouseenter", ".skPlayer-control", function () {
    $(".skPlayer-percent").css({
        "opacity": 1
    })
});
...
layui.config({
    base: window.location.origin + "/templates/perfree-simple-pro/static/plugins/laynotice/"
});
var lay_notice;
...
function formatMsgTime() {
    var dateTimeStamp = Date.parse($("#articleLastTime").text().replace(/-/gi, "/"));
    var minute = 1000 * 60;
    var hour = minute * 60;
    var day = hour * 24;
    var month = day * 30;
...
function shareToQzone(event, pic) {
    ...
    _shareUrl += "url=" + encodeURIComponent(location.href);
    _shareUrl += "&showcount=" + 1;
    _shareUrl += "&desc=" + encodeURIComponent(share_desc);
    _shareUrl += "&title=" + encodeURIComponent(document.title);
    _shareUrl += "&pics=" + encodeURIComponent(pic);
    window.open(_shareUrl, "_blank")
}

function shareToSinaWB(event, pic) {
    event.preventDefault();
    var _shareUrl = "http://v.t.sina.com.cn/share/share.php?";
    _shareUrl += "&url=" + encodeURIComponent(document.location);
    _shareUrl += "&title=" + encodeURIComponent(document.title);
    _shareUrl += "&content=" + "utf-8";
    _shareUrl += "&pic=" + encodeURIComponent(pic);
    window.open(_shareUrl, "_blank")
};