...
@section('title', '博客管理系统-文章列表')
@section('keywords', '博客管理系统-文章列表')
                            ...
                            <div class="col-sm-3">
                                <select class="input-sm form-control input-s-sm inline" name="sort_id">
                                    ...
                                    @foreach($sort as $key=>$val)
                                        <option value="{{ $val['id'] }}"
                                                @if($val['id']==$search_data['sort_id']) selected @endif>{{ $val['name'] }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-sm-3">
                                <div class="input-group">
                                    <input type="text" placeholder="请输入搜索内容" name="title"
                                           value="{{ $search_data['title'] }}" class="input-sm form-control">
                            ...
                            <tr>
                                <th>
                                    <label class="i-checks">
                                        <input type="checkbox"
                                               class="i-checks checkbox_module_name checkbox_module_name_1"
                                               value="1">
                                    </label>
                                </th>
                                <th>ID</th>
                                    ...
                                    <tr>
                                        <td>
                                            <label class="checkbox-inline i-checks"
                                                ...
                                                <input type="checkbox" data-group_id="{{ $val['id'] }}"
                                                       class="checkbox_node_name checkbox_node_name_{{ $val['id'] }}"
                                                       name="module_node_ids[]" value="{{ $val['id'] }}">
                                            </label>
                                        </td>
                                        <td>{{$val['id']}}</td>
                                        <td>{{$val['title']}}</td>
                                        ...
                                        <td>
                                            <button class="btn btn-info" type="button"
                                                    onclick="edit('{{$val['id']}}')"><i class="fa fa-edit"></i>&nbsp;&nbsp;编辑
                                            </button>
                                            <button class="btn btn-danger" type="button"
                                                    onclick="deleted('{{$val['id']}}')"><i
