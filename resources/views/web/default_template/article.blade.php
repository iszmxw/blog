                    ...
                    <div class="col-md-6">
                        @foreach($blog['tags'] as $value)
                            ...
                            <button class="btn btn-primary btn-xs" type="button"
                                    onclick="window.location.href='{{url('tag').'/'.$value['id']}}'">{{$value['name']}}</button>
                        @endforeach
                    </div>
                    <div class="col-md-6">
                                ...
                                <div class="ibox">
                                    <div class="ibox-content">
                                        ...
                                        <form action="{{url('blog/api/comment')}}" id="currentForm">
                                            <input type="hidden" name="gid" value="{{ $blog['id'] }}">
                                            {{csrf_field()}}
                                            <div class="form-group">
                                                <label>您的QQ</label>
                                                <input type="number" class="form-control" name="qq" id="qq"
                                                       placeholder="留下你的QQ号，方便联系！" onblur="check_user()">
                                            </div>
                                            <div class="form-group">
                                                <label>您的主页地址</label>
                                            ...
                                            <div class="form-group">
                                                <label>内容</label>
                                                <textarea class="form-control" name="comment" placeholder="发表意见"
                                                          rows="5"></textarea>
                                            </div>
                                            <div class="text-right">
                                                <button type="button" class="btn btn-sm btn-primary m-t-n-xs"
                                                        onclick="postForm()"><strong>发表评论</strong></button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
                                    ...
                                    <div class="social-feed-box">
                                        <div class="social-avatar">
                                            <a href="{{$val['url']}}" class="pull-left">
                                                @if($val['mail'])
                                                    <img alt="image"
                                                         src="http://q1.qlogo.cn/g?b=qq&nk={{$val['mail']}}&s=640">
                                                @else
